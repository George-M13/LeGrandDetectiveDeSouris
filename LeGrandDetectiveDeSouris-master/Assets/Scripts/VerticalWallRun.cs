...
public class VerticalWallRun : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (Input.GetKey(KeyCode.Mouse1) && (pm.currentState == PlayerMovement.PlayerState.climbing || pm.currentState == PlayerMovement.PlayerState.wallgrab))
        {
        ...
        if (pm.currentState == PlayerMovement.PlayerState.climbing && !Physics.Raycast(new Vector3(this.transform.position.x, this.transform.position.y-0.5f, this.transform.position.z), transform.forward, out wallHit, 1f))
        {
